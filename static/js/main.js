document.addEventListener('DOMContentLoaded', function() {
    ...
    if (token) {
        document.querySelectorAll('.auth-links').forEach(el => el.classList.add('hidden'));
        document.querySelectorAll('.user-links').forEach(el => el.classList.remove('hidden'));
        document.querySelectorAll('.new-thread-btn').forEach(el => el.classList.remove('hidden'));
    }
    ...
    // Charger les threads sur la page threads.html
    const threadList = document.getElementById('thread-list');
    if (threadList) {
        fetchThreads();
    }

    // Modal pour nouveau thread
    ...
    const searchInput = document.getElementById('search-thread');
    const filterTag = document.getElementById('filter-tag');

    if (searchInput) {
        searchInput.addEventListener('input', function() {
            fetchThreads(searchInput.value, filterTag.value);
        });
    }
